...
if __name__ == "__main__":
    ...
    for i in range(1, 50):
        print('已读取%d本小说' % (i*20))
        books = get_book_info(init_url.format(i))
        df = df.append(books, ignore_index=True)
    # 存储到excel
    df.to_excel('./top1000_books.xlsx')

    df = pd.read_excel('./top1000_books.xlsx')
    # 过滤掉评分低于7分，太监，评论数小于500的作品
    df1 = df[(df['score'] >= 7.0) & (df['status'] != 2)
             & (df['scorerCount'] >= 500)]
    # 截取更新时间的年份
    df1['updateAt'] = df1['updateAt'].map(lambda x: x[:4])
    # 把2020没更新的伪连载作品找出来
    df1['flag'] = df1.apply(lambda row: (int(row['updateAt']) < 2020) & (row['status'] == 0), axis=1)
    df1 = df1[df1['flag'] == False]

    writer = pd.ExcelWriter('./filter_books.xlsx')
    df1.to_excel(writer)
    writer.save()
